...
<body>
  ...
  .entryTable th, .entryTable td {
    ...
    //if($_SERVER["REQUEST_METHOD"]=="POST" && isset($_POST["id"], $_POST["name"])){//check field exists in the form submit
    if($_SERVER["REQUEST_METHOD"]=="POST" && trim($_POST["id"]) && !empty(trim($_POST["name"]))){
...
/*********** */
  ...
  if (!empty($_SESSION['entries'])) {//Check field is not empty
/*********** */
      echo 
        "<table>
        <thead >
